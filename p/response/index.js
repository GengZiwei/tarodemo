...
Page({
  ...
  value: {
    ...
    navOrderTime: 0, // 下单等到时间
  },
  globalData: app.globalData,
  onLoad: function(options){
    this.value.options = options;
    this.value.navOrderTime = (+app.globalData.basicInformationList.cancelTime || 3 * 60) + 30;
    this.mapCtx = wx.createMapContext('myMap');
    options.orderTaking && this.getorder(options.orderId)
    options.number&& this.stopOrder(app.globalData.starword);
  },
  onReady: function(){
    this.setData({
  ...
  onShow(){
    ...
    webSocket.connectSocket({
      success: function(){
        console.log('接驾soket开始', new Date((new Date().getTime())).Format('yyyy-MM-dd hh:mm:ss.S'))
      }
    });
  ...
  getorder(orderid, tosoket){
    ...
    http({
      ...
      if(orderInfo.status == 2) {
        ...
        that.value.mayStartTime = orderInfo.mayStartTime
        times <= 0 ? times = 0  : times = parseInt(times / 1000)
        tt = times
        that.countdown()
        if(new Date().getTime() - orderInfo.mayStartTime >= +app.globalData.basicInformationList.noResTime * 1000) {
          this.setData({
            carArrive: true,
        ...
        if(!this.data.carArrive) {
          ...
          if(tt > 0) {
            ...
            this.threeMinutes = setTimeout(() => { // 定时器开始提示晚点补偿
              this.setData({
                carArrive: true,
                goCarArriveTitle: '已为您开启晚点补偿'
              })
            }, (+app.globalData.basicInformationList.noResTime * 1000) - (new Date().getTime() - orderInfo.mayStartTime));
          }
        }
        ...
        that.value.options.number = orderInfo.peopleNumber
        that.globalData = Object.assign(that.globalData,{
          starPoint:{
            location: orderInfo.startGps,
            name: orderInfo.startPlace
          },
          endPoint: {
            location: orderInfo.endGps,
            name: orderInfo.endPlace
          },
          starword: {
            typeID: orderInfo.startStationId
          },
          endword: {
            typeID: orderInfo.endStationId
          }
        })
        that.stopOrder({
          longitude: orderInfo.stationStartGps.split(',')[0],
          latitude: orderInfo.stationStartGps.split(',')[1],
          name: orderInfo.stationStartPlace
        }, that.value.options.time || that.value.navTime)
      } else {
    ...
    let data = JSON.parse(res)
      setTimeout(() => {
        if(data.type == 'cancel') { // 取消订单
          ...
          console.log(data.value, that.value.navOrderID)
          clearInterval(interval);
          interval = ''
          tt = 0
          that.value.navTime = ''
  ...
  closeOrder: function() {
    ...
    payState = false,
    navOrder = false,
    {orderId, time: noTime} = that.value.options,
    ...
    if(orderId && !noTime) {
      let time = new Date().getTime() - that.value.mayStartTime;
      ...
      success(res) {
        if(res.confirm && that.value.options.orderId && navOrder){
          ...
          res.confirm && http({
            ...
            navOrder ? wx.navigateBack() : app.navigateTo({
